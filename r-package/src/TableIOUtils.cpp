...
IOTable tableFromDataFrame(DataFrame df){
  IOTable retTab;
  CharacterVector colNames = df.names();
  
  for(int i = 0; i< df.length(); i++){
    retTab.addColumn(as<string>(colNames[i]), as<vector<string> >(df[i]));
  }
  return(retTab);
}

DataFrame tableToDataFrame(IOTable tab){
  DataFrame df = DataFrame::create();
  vector <string> colNames = tab.getColNames();
  
  for(int i = 0; i < tab.getNumCols(); i++) {
    df.push_back(tab.getCol(i), colNames[i]);
  }
  return(DataFrame(df));
}
